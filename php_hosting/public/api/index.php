...
// ext-curl isn't installed on this host, so this goes through the http:// stream wrapper
// (needs ext-openssl for the https:// scheme) instead of curl.
function telegram_notify_managers($text) {
    if (!defined('TELEGRAM_BOT_TOKEN') || !defined('TELEGRAM_MANAGER_GROUP_ID')) return false;
    $payload = json_encode(array('chat_id' => TELEGRAM_MANAGER_GROUP_ID, 'text' => $text));
