...
const mkBlock=()=>({id:uid('b'),title:'Новый блок',elements:[]});
...
function renderOptions(){
  ...
  groups.forEach(g=>{
    ...
    list.forEach(o=>{
      const d=document.createElement('div');
      d.className='item';
      d.innerHTML=`<div style="min-width:0"><b style="display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(o.name)}</b><div class="tiny">${esc(o.group_name||g.name)} · ${Number(o.price).toLocaleString('ru-RU')} ₽</div></div><div style="display:flex;gap:4px;align-items:center;flex-shrink:0"><button class="icon-btn" data-edit-opt="${o.id}" title="Редактировать">✎</button><button class="icon-btn" data-del-opt="${o.id}" title="Удалить">🗑</button></div>`;
      $('options').appendChild(d);
      d.querySelector('[data-edit-opt]').onclick=()=>editOption(o);
      d.querySelector('[data-del-opt]').onclick=()=>deleteOption(o);
      const c=document.createElement('article');
      c.className='card';
      c.innerHTML=`${o.image_url?`<img src="${o.image_url}" style="height:130px">`:''}<div style="padding:8px"><b>${esc(o.name)}</b><div class="tiny">${esc(o.group_name||g.name)} · ${Number(o.price).toLocaleString('ru-RU')} ₽</div></div>`;
...
function renderWorkspace(){
  ...
  S.page.blocks.forEach((b,i)=>{const box=document.createElement('div');box.className='block'+(S.selectedBlockId===b.id?' sel':'');box.dataset.blockId=b.id;box.style.background=b.backgroundColor||'#ffffff';box.onclick=()=>{S.selectedBlockId=b.id;S.selectedElId=null;renderWorkspace();renderSettings();};
    let html=`<div><b>${esc(b.title||'Блок')}</b></div>`;
    (b.elements||[]).forEach(e=>{
      ...
      if(e.type==='image'){const imgSrc=normalizeUrl(e.url||e.imageUrl||'');html+=`<div class="el" data-e="${e.id}"><div class="photo" data-image-drop="${e.id}" style="height:${e.height||320}px">${imgSrc?`<img src="${imgSrc}" style="opacity:${(e.opacity||100)/100};filter:contrast(${e.contrast||100}%) brightness(${e.brightness||100}%)">`:'<div class="tiny" style="padding:12px">Перетащите фото сюда</div>'}${(e.texts||[]).map((t,ti)=>`<div class="text-box" data-img-text-box="${e.id}" data-ti="${ti}" style="left:${t.x||8}%;top:${t.y||8}%;width:${t.width||55}%;color:${t.color||'#fff'};font-size:${t.size||34}px;${fontCss(t.fontKey||'inter')};opacity:${(t.opacity||100)/100};filter:contrast(${t.textContrast||100}%) brightness(${t.textBrightness||100}%) saturate(${t.textSaturation||100}%);text-shadow:0 2px 12px rgba(0,0,0,${(t.shadow||35)/100});transform:rotate(${t.rotate||0}deg)"><i class="move" data-action="move" data-e="imgtxt:${e.id}:${ti}"></i><div contenteditable="true" data-edit-img-text="${e.id}" data-ti="${ti}" spellcheck="false">${esc(t.text||'')}</div><i class="rotate" data-action="rotate" data-e="imgtxt:${e.id}:${ti}">${ROTATE_ICON}</i><i class="resize" data-action="resize" data-e="imgtxt:${e.id}:${ti}"></i>${(Math.abs(parseFloat(t.rotate||0))<0.01)?'<span class="angle-zero">0°</span>':''}</div>`).join('')}</div></div>`;}
      if(e.type==='form'){html+=`<div class="el" data-e="${e.id}"><div class="tiny">Форма: ${esc(e.title||'Заявка')}</div></div>`;}
      if(e.type==='options'){const cols=clamp(parseInt(e.columns||3,10),1,6);const arr=S.options.filter(o=>(e.optionIds||[]).includes(parseInt(o.id,10)));html+=`<div class="el" data-e="${e.id}"><div class="cards" style="grid-template-columns:repeat(${cols},minmax(0,1fr));gap:${e.cardGap||10}px">${arr.map(o=>{const meta=optionMeta(o);return `<article class="card" style="border:${e.cardBorder||1}px solid var(--l);border-radius:${e.cardRadius||8}px;box-shadow:0 ${e.cardShadow||0}px ${(e.cardShadow||0)*3}px rgba(15,23,42,.12)">${isWoodColor(o)?`<div class="wood-sample" style="--wood:${meta.hex||'#ddd'};height:${e.cardSize||120}px"></div>`:(o.image_url?`<img src="${o.image_url}" style="height:${e.imageHeight||160}px">`:'')}<div style="padding:8px;font-size:${e.textSize||14}px"><b>${esc(o.name)}</b><div class="tiny">${esc(o.group_name||'')}</div><div class="tiny">${Number(o.price).toLocaleString('ru-RU')} ₽</div></div></article>`}).join('')}</div></div>`;}
    });
    box.innerHTML=html;
    box.querySelectorAll('[data-e]').forEach(n=>n.onclick=(ev)=>{ev.stopPropagation();S.selectedBlockId=b.id;S.selectedElId=n.getAttribute('data-e');if(ev.target.closest('[contenteditable="true"]')){renderSettings();return;}renderWorkspace();renderSettings();});
...
function renderSettings(){
  ...
  if(!it){
    ...
    <div class="field"><label>Название блока</label><input value="${esc(b.title||'')}" onchange="setBlockTitle(this.value)"></div>
    <div class="field"><label>Цвет фона блока</label>${colorPickerHtml(b.backgroundColor||'#ffffff',"setBlockBg({v})")}<input value="${esc(b.backgroundColor||'#ffffff')}" placeholder="#ffffff / rgb(255,255,255)" onchange="setBlockBg(this.value)"></div>
    ...
    <div style="margin-bottom:8px"><button class="btn" style="width:100%;border-color:#ef9a9a;color:#a11" onclick="removeBlock()">Удалить блок</button></div>
  ...
  if(it.type==='text'){
    p.innerHTML=`${quickMenu}<div style="margin-bottom:8px"><button class="btn" style="width:100%;border-color:#ef9a9a;color:#a11" onclick="removeBlock()">Удалить блок</button></div><details open><summary><b>Текст</b></summary>
  ...
  if(it.type==='image'){
    ...
    p.innerHTML=`${quickMenu}<div style="margin-bottom:8px"><button class="btn" style="width:100%;border-color:#ef9a9a;color:#a11" onclick="removeBlock()">Удалить блок</button></div><details open><summary><b>Фон</b></summary>
      <div style="margin-top:8px">
        <div class="drop" id="dropZone">Перетаскивание временно отключено. Выберите файл ниже.</div>
        <div style="display:flex;gap:8px;align-items:center;margin:8px 0">
          <input id="fileInput" type="file" accept="image/*">
          <button class="btn" id="uploadBtn" type="button">Загрузить файл</button>
        </div>
        <div class="field"><label>Выбрать из медиатеки</label>
        ...
        <div class="field"><label>URL фото</label><input value="${esc(it.url||it.imageUrl||'')}" onchange="setEl('url',this.value)"></div>
        <div class="field"><label>Высота блока фото</label><input type="range" min="180" max="800" value="${it.height||320}" oninput="setEl('height',parseInt(this.value,10))"></div>
        <div class="field"><label>Прозрачность фото</label><input type="range" min="0" max="100" value="${it.opacity||100}" oninput="setEl('opacity',parseInt(this.value,10))"></div>
        <div class="field"><label>Контраст фото</label><input type="range" min="50" max="180" value="${it.contrast||100}" oninput="setEl('contrast',parseInt(this.value,10))"></div>
      </div>
    </details>
    <details ${S.imageTextOpen?'open':''}><summary><b>Текст</b></summary>
      <div style="margin-top:8px">
        ...
        <div style="display:flex;gap:6px;margin-bottom:8px">
          <button class="btn" type="button" onclick="addImgText()">Добавить еще текст</button>
          ${texts.length?'<button class="btn" type="button" onclick="removeImgText()">Удалить текст</button>':''}
        </div>
  ...
  if(it.type==='form'){
    p.innerHTML=`${quickMenu}<div style="margin-bottom:8px"><button class="btn" style="width:100%;border-color:#ef9a9a;color:#a11" onclick="removeBlock()">Удалить блок</button></div><div class="field"><label>Заголовок формы</label><input value="${esc(it.title||'Заявка')}" oninput="setEl('title',this.value)"></div>`;
    return;
  }
  if(it.type==='options'){
    ...
    p.innerHTML=`${quickMenu}<div style="margin-bottom:8px"><button class="btn" style="width:100%;border-color:#ef9a9a;color:#a11" onclick="removeBlock()">Удалить блок</button></div><div class="field"><label>Опций в ряд</label><input type="range" min="1" max="6" value="${it.columns||3}" oninput="setEl('columns',parseInt(this.value,10))"></div>
    <div class="field"><label>Высота картинки</label><input type="range" min="100" max="320" value="${it.imageHeight||160}" oninput="setEl('imageHeight',parseInt(this.value,10))"></div>
    <div class="field"><label>Размер карточки цвета</label><input type="range" min="80" max="240" value="${it.cardSize||120}" oninput="setEl('cardSize',parseInt(this.value,10))"></div>
    <div class="field"><label>Расстояние между карточками</label><input type="range" min="4" max="32" value="${it.cardGap||10}" oninput="setEl('cardGap',parseInt(this.value,10))"></div>
    <div class="field"><label>Скругление</label><input type="range" min="0" max="24" value="${it.cardRadius||8}" oninput="setEl('cardRadius',parseInt(this.value,10))"></div>
    ...
    <div class="field"><label>Размер текста</label><input type="range" min="12" max="24" value="${it.textSize||14}" oninput="setEl('textSize',parseInt(this.value,10))"></div>
    <div class="field"><label>Выбор опций</label><div style="border:1px solid #e5e7eb;border-radius:8px;padding:8px;max-height:220px;overflow:auto">${checks||'<span class="tiny">Сначала добавьте опции.</span>'}</div></div>`;
  }
}
function bindUpload(){
...
$('newModel').onclick=async()=>{const name=prompt('Название модели'); if(!name)return; const fd=new URLSearchParams(); fd.append('name',name); const r=await api('./api/index.php?action=create_model',{method:'POST',body:fd}); if(r.ok){await boot(); await loadPage(r.id)}};
