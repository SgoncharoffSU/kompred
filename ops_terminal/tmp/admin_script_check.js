...
function renderWorkspace(){
  ...
  S.page.blocks.forEach((b,i)=>{const box=document.createElement('div');box.className='block'+(S.selectedBlockId===b.id?' sel':'');box.dataset.blockId=b.id;box.onclick=()=>{S.selectedBlockId=b.id;S.selectedElId=null;renderWorkspace();renderSettings();};
    ...
    (b.elements||[]).forEach(e=>{
      ...
      if(e.type==='options'){const cols=clamp(parseInt(e.columns||3,10),1,5);const arr=S.options.filter(o=>(e.optionIds||[]).includes(parseInt(o.id,10)));html+=`<div class="el" data-e="${e.id}"><div class="cards" style="grid-template-columns:repeat(${cols},minmax(0,1fr))">${arr.map(o=>`<article class="card">${o.image_url?`<img src="${o.image_url}" style="height:${e.imageHeight||160}px">`:''}<div style="padding:8px;font-size:${e.textSize||14}px"><b>${esc(o.name)}</b><div class="tiny">${Number(o.price).toLocaleString('ru-RU')} ₽</div></div></article>`).join('')}</div></div>`;}
    });
    box.innerHTML=html;
...
function renderSettings(){
  ...
  if(it.type==='options'){
    ...
    p.innerHTML=`${quickMenu}<div style="margin-bottom:8px"><button class="btn" style="width:100%;border-color:#ef9a9a;color:#a11" onclick="removeBlock()">Удалить блок</button></div><div class="field"><label>Опций в ряд</label><input type="range" min="1" max="5" value="${it.columns||3}" oninput="setEl('columns',parseInt(this.value,10))"></div>
